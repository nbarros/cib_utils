...
#define I2C_RESET       0x7
  ...
  int32_t i2c_write_smbus(const device &dac, uint8_t reg, uint16_t value, bool debug = false) {
    ...
    if (debug) {
      printf("==================================================================\n");
      printf("i2c_write_smbus : Writing contents to I2C DAC %hu:\n",dac.channel);
      printf(" * device : %s [fd %i] [DAC %hu 0x%X]\n",dac.devname.c_str(),
                                                      dac.fd,
                                                      static_cast<uint16_t>(dac.channel),dac.addr);
  ...
  int32_t i2c_read_smbus(const device &dac, uint8_t reg, uint16_t &value, bool debug = false) {
    ...
    if (debug) {
      printf("==================================================================\n");
      printf("i2c_read_smbus : Reading contents to I2C DAC %hu:\n",static_cast<uint16_t>(dac.channel));
      printf(" * device : %s [fd %i] [DAC %i 0x%X]\n",dac.devname.c_str(),
                                                      dac.fd,
...
void dac_galore(){
  ...
  for (size_t i = 0; i < ndacs; i++) {
    ...
    for (size_t j = 0; j < nchannels; j++) {
      ...
      printf("%04hu ",threshold_table[i][j]);
    }
    printf("\n");
  }
