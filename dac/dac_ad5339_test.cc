...
#define I2C_RESET       0x7
  ...
  const size_t ndacs = 1;
  uint8_t DAC[ndacs] = {0x0D};
  
  int32_t i2c_init( int32_t &fd,const std::string &device = "/dev/i2c-8", bool debug = true) 
  {
    int32_t file = -1;
    int32_t res = -1;
    if (debug) {
...
int main() {
  ...
  c.cmd.channel = ALL_CH;  

  printf("Resetting all ch %u : %u (0x%X)\n",c.cmd.channel,0x0,0x0);
  // -- Write a new level on this same channel
  c.cmd.command = I2C_WR_UPD_N;
  l.set_int_level(0x0); // -- the function should swap the bytes automatically
  // putting in the contents as 0xC0A3, which is what the i2c wants 
  // damn endianess...
  if (!i2c::set_dac_level(dev,c,l,true)) {
    printf("Failed to write\n");
  ...
  printf("Setting level for all channels %u : %u (0x%X)\n",c.cmd.channel,0x75F,0x75F);
  // -- Write a new level on this same channel
  c.cmd.command = I2C_WR_UPD_ALL;
  l.set_int_level(0x75F); // -- the function should swap the bytes automatically
  // putting in the contents as 0xC0A3, which is what the i2c wants 
  // damn endianess...
  if (!i2c::set_dac_level(dev,c,l,true)) {
    printf("Failed to write\n");
