...
static int vc5_update_bits(struct udevice *dev, unsigned int reg, unsigned int mask, unsigned int src)
{
	int ret;
	unsigned char cache;
...
static unsigned long vc5_fod_set_rate(struct clk *hw, unsigned long rate)
{
	 ...
	 */
	vc5_update_bits(vc5->i2c, VC5_GLOBAL_REGISTER,VC5_GLOBAL_REGISTER_GLOBAL_RESET, 0);
	vc5_update_bits(vc5->i2c, VC5_GLOBAL_REGISTER,VC5_GLOBAL_REGISTER_GLOBAL_RESET,VC5_GLOBAL_REGISTER_GLOBAL_RESET);

	// this is toggling the reset down and up again
	// this would in principle mean 
	// int vc5_update_bits(*dev, reg, mask, src)
	// vc5_update_bits(vc5->i2c,0x76,BIT(5),0);
	// vc5_update_bits(vc5->i2c,0x76,BIT(5),BIT(5));
